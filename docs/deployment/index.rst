...
b) A set of global (not environment specific) configuration is set bellow the environment mapping. Things like error reporting, the default mail_system, default caching options, zip compression, fast_404, and many settings more.

c) Environment specific happens after b) enclosed in a switch statement that analyses the ENVIRONMENT constant:
