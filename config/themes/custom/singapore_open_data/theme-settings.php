 ...
 */
function singapore_open_data_form_system_theme_settings_alter(&$form, &$form_state) {
  global $theme;
  // Ensure this include file is loaded when the form is rebuilt from the cache.
  $form_state['build_info']['files']['form'] = drupal_get_path('theme', 'singapore_open_data') . '/theme-settings.php';
  ...
  $copyright = theme_get_setting('copyright', 'singapore_open_data');
  ...
  // Log In Display.
  $display_login_menu = (theme_get_setting('display_login_menu', 'singapore_open_data') === NULL) ? 1 : theme_get_setting('display_login_menu', 'singapore_open_data');
  ...
  // Upload field.
  $hero = theme_get_setting('hero_file', 'singapore_open_data');
  ...
  $form['hero']['background_option'] = array(
    ...
    '#required' => FALSE,
    '#default_value' => theme_get_setting('background_option', 'singapore_open_data'),
    '#element_validate' => array('_singapore_open_data_background_option_setting'),
  );
 ...
 */
function _singapore_open_data_background_option_setting($element, &$form, &$form_state) {
 ...
 */
function singapore_open_data_hero_system_theme_settings_form_submit(&$form, &$form_state) {
  if ($form_state['values']['hero_file']) {
    $fid = $form_state['values']['hero_file'];
    singapore_open_data_file_set_permanent($fid);
  }
 ...
 */
function singapore_open_data_file_set_permanent($fid) {
  $file = file_load($fid);
  $file->status = FILE_STATUS_PERMANENT;
  file_save($file);
  file_usage_add($file, 'theme', 'file', $fid);
  singapore_open_data_file_insert($file);
}

/**
* Implements hook_file_insert().
*/
function singapore_open_data_file_insert($file) {
